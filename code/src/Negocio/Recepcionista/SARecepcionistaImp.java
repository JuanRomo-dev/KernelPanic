...
public class SARecepcionistaImp implements SARecepcionista {
	...
	public Integer editar(TRecepcionista tRecepcionista) {
		...
		Recepcionista recepcionista = em.find(Recepcionista.class, tRecepcionista.getId(),LockModeType.);
		List<Recepcionista> check_nombre = em.createNamedQuery("Negocio.Recepcionista.Recepcionista.findBynombre", Recepcionista.class).setParameter("nombre", recepcionista.getNombre()).getResultList();
		if (recepcionista != null && recepcionista.getActivo() && (check_nombre == null || check_nombre.isEmpty())) {
		...
		else if(check_nombre != null || !check_nombre.isEmpty()){
			res = -2;
			transaction.rollback();
		}
		else {
