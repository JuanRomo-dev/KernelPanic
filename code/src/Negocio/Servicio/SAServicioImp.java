...
public class SAServicioImp implements SAServicio {
	...
	public Integer editar(TServicio tServicio) {
		...
		List<Servicio> check_tipo = em.createNamedQuery("Negocio.Servicio.Servicio.findBytipo", Servicio.class).setParameter("tipo", servicio.getTipo()).getResultList();
		
		if (servicio != null && servicio.getActivo() && (check_tipo == null || check_tipo.isEmpty())) {
			em.lock(servicio, LockModeType.OPTIMISTIC);
		...
		else if(check_tipo != null || !check_tipo.isEmpty()){
			res = -2;
			transaction.rollback();
		}
		else {
